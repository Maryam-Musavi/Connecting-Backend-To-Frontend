...
import { Mail, MapPin, Linkedin, Phone, Printer, Send, RotateCcw, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const ContactView: React.FC<ContactViewProps> = ({ lang }) => {
  ...
  const validate = (): boolean => {
    ...
    } else if (!/^[0-9+()\-\s]{6,25}$/.test(form.phone)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) {
      setStatus('error');
      setStatusMessage(lang === 'fa' ? 'برخی فیلدها نیازمند بازبینی هستند.' : 'Some fields need your attention.');
      return;
    }
    setStatus('submitting');
    setStatusMessage('');
    try {
      const payload = { ...form, lang, website_url_hp: '' };
  ...
  return (
    <div className="space-y-20 pb-24">
      ...
      {/* MAIN 12-COLUMN SWISS LAYOUT */}
      <section className="max-w-[1440px] mx-auto px-6 sm:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          
          {/* BUSINESS OPPORTUNITY INTRO: 7 COLS */}
          <div className="lg:col-span-7 bg-[#071927] border border-white/10 p-8 sm:p-12 space-y-6">
            <h2 className="text-xl sm:text-2xl font-light text-white tracking-wide">
              {t.opportunityHeading}
            </h2>
            <div className="h-px w-12 bg-[#00C4CC]" />
            <div className="text-sm sm:text-base text-slate-300 font-light leading-relaxed space-y-4 text-justify">
              {t.opportunityText.split('\n\n').map((paragraph, idx) => (
                <p key={idx}>{paragraph}</p>
              ))}
            </div>
            <div className="pt-4 border-t border-white/10">
              <a
                href="mailto:info@refah-spc.ir"
              ...
              >
                ...
                <span>{lang === 'fa' ? 'مکاتبه با ایمیل info@refah-spc.ir' : 'Email info@refah-spc.ir'}</span>
              </a>
            </div>
          </div>

          {/* CONTACT INFO & MAP LOCATION: 5 COLS */}
          <div className="lg:col-span-5 space-y-8">
            
            {/* Direct Details Box */}
            <div className="p-8 bg-[#071927] border border-white/10 space-y-6">
              <div className="border-b border-white/10 pb-3">
                <span className="text-[10px] font-mono text-[#00C4CC] uppercase tracking-widest block">
                  {t.commercialCommTitle}
                </span>
              ...
              <div className="space-y-6 text-xs font-mono text-slate-300">
                <div className="flex items-start gap-5 rtl:gap-8">
                  <Mail className="w-4 h-4 text-[#00C4CC] shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-500 block text-[10px] mb-1.5">{t.emailTitle.toUpperCase()}</span>
                    <a href="mailto:info@refah-spc.ir" dir="ltr" className="hover:text-[#00C4CC] transition-colors font-mono inline-block">
                      info@refah-spc.ir
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-5 rtl:gap-8">
                  <Phone className="w-4 h-4 text-[#00C4CC] shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-500 block text-[10px] mb-1.5">{t.phoneTitle.toUpperCase()}</span>
                    <a href="tel:+982166590292" dir="ltr" className="hover:text-[#00C4CC] transition-colors font-mono inline-block">
                      {t.phone}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-5 rtl:gap-8">
                  <Printer className="w-4 h-4 text-[#00C4CC] shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-500 block text-[10px] mb-1.5">{t.faxTitle.toUpperCase()}</span>
                    <a href="tel:+982166923081" dir="ltr" className="hover:text-[#00C4CC] transition-colors font-mono inline-block">
                      {t.fax}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-5 rtl:gap-8">
                  <MapPin className="w-4 h-4 text-[#00C4CC] shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-500 block text-[10px] mb-1.5">{t.addressTitle}</span>
                ...
                <div className="flex items-start gap-5 rtl:gap-8 pt-2 border-t border-white/5">
                  <Linkedin className="w-4 h-4 text-[#00C4CC] shrink-0 mt-0.5" />
                  <div>
                    <span className="text-slate-500 block text-[10px] mb-1.5">{t.networkTitle.toUpperCase()}</span>
                    <a
                      href="https://www.linkedin.com/in/refah-sanat-pardis-company-61222a427"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="hover:text-[#00C4CC] transition-colors underline underline-offset-4"
                    >
                      {t.linkedIn}
                    </a>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-3">
                  {summaryRows.map((row, idx) => (
                    <div key={idx} className="flex flex-col border-b border-white/5 pb-2">
                      ...
                      <span className="text-sm text-slate-200 break-words whitespace-pre-wrap" dir="auto">
                        {row.value}
                      </span>
                    </div>
                  ))}
